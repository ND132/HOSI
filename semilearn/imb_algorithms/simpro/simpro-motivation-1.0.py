    ...
    def train(self):
        ...
        for epoch in range(self.start_epoch, self.epochs):
            ...
            if self.epoch > 0 and self.epoch % 5 == 0:
                ...
                ax.set_ylim(0, max(max(np.array(self.ulb_dist.cpu())), max(np.array(record_mask_true) + np.array(record_mask_false))) * 1.1)
                ...
                sample_indices = np.arange(0, max(max(np.array(self.ulb_dist.cpu())), max(np.array(record_mask_true) + np.array(record_mask_false))) * 1.1, 1000)

                ax.set_yticks(sample_indices)
                ax.set_yticklabels([f'{int(i)}' for i in sample_indices], fontsize=18, rotation=0)

                ax.legend(loc='upper center', bbox_to_anchor=(0.9, 1.0), ncol=1, handlelength=2.5, handleheight=0.8, borderpad=0.1, columnspacing=0.8, fontsize=16, framealpha=0.2)
