...
class FreeMatch(ImbAlgorithmBase):
    ...
    def train_step(self, idx_lb, x_lb_w, x_lb_s, y_lb, y_lb_noised, idx_ulb, x_ulb_w, x_ulb_s, y_ulb):
        ...
        with self.amp_cm():
            ...
            x_r = torch.cat([torch.rot90(x_ulb_w[:num_lb], i, [2, 3]) for i in range(4)], dim=0)
            y_r = torch.cat([torch.empty(x_ulb_w[:num_lb].size(0)).fill_(i).long() for i in range(4)], dim=0).cuda(self.gpu)

            rot_logits = self.model(x_r)['rot']

            rot_loss = self.ce_loss(rot_logits, y_r, reduction='mean')
            ...
            total_loss = sup_loss + self.lambda_u * (unsup_loss + rot_loss) + self.lambda_e * ent_loss
