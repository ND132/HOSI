...
class MetaExpertNet(nn.Module):
    ...
    def forward(self, x, **kwargs):
        ...
        results_dict['final_feat'] = feat444
...
class MetaExpert(ImbAlgorithmBase):
    ...
    def train_step(self, idx_lb, x_lb_w, x_lb_s, y_lb, y_lb_noised, idx_ulb, x_ulb_w, x_ulb_s, y_ulb):
        #x_lb, y_lb, y_lb_noised, x_ulb_w, x_ulb_s, y_ulb
        x_lb = x_lb_w
    ...
    def extract_features_small(self, eval_dest='eval', layer_name='feat', save_path=None):
        ...
        with torch.no_grad():
            for data in eval_loader:
                ...
                # logits
                w1 = outputs['w1'].reshape(-1, 1)
                w2 = outputs['w2'].reshape(-1, 1) 
                w3 = outputs['w3'].reshape(-1, 1)
            
                logit1 = outputs['logits']
                logit2 = outputs['aux_logits1']
                logit3 = outputs['aux_logits2']
            
                # logits
                weighted_logits = w1 * logit1 + w2 * logit2 + w3 * logit3
                _, preds = torch.max(weighted_logits, 1)
                pred_labels.append(preds.cpu().numpy())
            
                # layer_name
                if layer_name == 'feat':
                    # feat4
                    #feat = outputs['feat_for_fuse']['feat4']
                    feat = outputs['final_feat']
                elif layer_name == 'logits':
                    feat = weighted_logits
                else:
        ...
        # 
        min_length = min(len(features), len(true_labels), len(pred_labels))
        if len(features) != min_length or len(true_labels) != min_length or len(pred_labels) != min_length:
            self.print_fn(f"WARNING: Array length mismatch - features: {len(features)}, true_labels: {len(true_labels)}, pred_labels: {len(pred_labels)}")
            self.print_fn(f"Truncating to minimum length: {min_length}")
        ...
        if self.num_classes <= 10:
            colors = plt.cm.tab10(np.linspace(0, 1, 10))[:self.num_classes]
        elif self.num_classes <= 20:
            colors = plt.cm.tab20(np.linspace(0, 1, 20))[:self.num_classes]
        else:
            colors = plt.cm.Set3(np.linspace(0, 1, min(12, self.num_classes)))
            if self.num_classes > 12:
                colors2 = plt.cm.Pastel2(np.linspace(0, 1, min(8, self.num_classes-12)))
                colors = np.vstack([colors, colors2])
                if self.num_classes > 20:
                    colors3 = plt.cm.Pastel1(np.linspace(0, 1, min(9, self.num_classes-20)))
                    colors = np.vstack([colors, colors3])
        ...
        # 
        scatter_true = ax1.scatter(df_true['x'], df_true['y'], 
                                  c=df_true['label'], cmap=cmap,
                                  alpha=0.7, s=7, 
                                  vmin=0, vmax=self.num_classes-1)
        #ax1.set_title(f'TSNE of {layer_name} Features (True Labels)')
        ...
        # 
        scatter_pred = ax2.scatter(df_pred['x'], df_pred['y'], 
                                  c=df_pred['label'], cmap=cmap,
                                  alpha=0.7, s=7,
                                  vmin=0, vmax=self.num_classes-1)
        #ax2.set_title(f'TSNE of {layer_name} Features (Predicted Labels)')
        ...
        if save_path is None:
            save_path = os.path.join(self.args.save_dir, self.args.save_name, f"tsne_{layer_name}_comparison.png")
            save_path_pdf = os.path.join(self.args.save_dir, self.args.save_name, f"tsne_{layer_name}_comparison.pdf")
        else:
            # Ensure save_path doesn't have extension
            save_path = os.path.splitext(save_path)[0] + '_comparison.png'
            save_path_pdf = os.path.splitext(save_path)[0] + '_comparison.pdf'

        plt.savefig(save_path, bbox_inches='tight', dpi=1000, transparent=True)  # transparent=True
        plt.savefig(save_path_pdf, bbox_inches='tight', dpi=1000, transparent=True)
        self.print_fn(f"TSNE comparison visualization saved to {save_path}")

        # Also save separate figures with discrete legends
        base_path = os.path.splitext(save_path)[0]

        # Save true labels plot
        fig_true, ax_true = plt.subplots(figsize=(11, 8))
        # 
        ...
        scatter_true = ax_true.scatter(df_true['x'], df_true['y'], 
                                  c=df_true['label'], cmap=cmap,
                                  alpha=0.7, s=5, 
                                  vmin=0, vmax=self.num_classes-1)
        #ax_true.set_title(f'TSNE of {layer_name} Features (True Labels)')
        #ax_true.legend(handles=legend_elements, 
        #           loc='center left', 
        #           bbox_to_anchor=(1, 0.5),
        #           title='Classes',
        #           frameon=False)
        ax_true.legend(handles=legend_elements, 
                   ...
                   frameon=False)
        plt.tight_layout()
        plt.savefig(f"{base_path}_true.png", bbox_inches='tight', dpi=1000, transparent=True)
        plt.savefig(f"{base_path}_true.pdf", bbox_inches='tight', dpi=1000, transparent=True)

        # Save predicted labels plot
        fig_pred, ax_pred = plt.subplots(figsize=(11, 8))
        # 
        ax_pred.spines['top'].set_visible(False)
        ax_pred.spines['right'].set_visible(False)
        ax_pred.spines['bottom'].set_visible(False)
        ax_pred.spines['left'].set_visible(False)
        ax_pred.tick_params(axis='both', which='both', length=0)
        ax_pred.set_xticks([])
        ax_pred.set_yticks([])
        
        scatter_pred = ax_pred.scatter(df_pred['x'], df_pred['y'], 
                                  c=df_pred['label'], cmap=cmap,
                                  alpha=0.7, s=5,
                                  vmin=0, vmax=self.num_classes-1)
        #ax_pred.set_title(f'TSNE of {layer_name} Features (Predicted Labels)')
        #ax_pred.legend(handles=legend_elements, 
        #           loc='center left', 
        #           bbox_to_anchor=(1, 0.5),
        #           title='Classes',
        #           frameon=False)
        ax_pred.legend(handles=legend_elements, 
                   ...
                   frameon=False)
        plt.tight_layout()
        plt.savefig(f"{base_path}_pred.png", bbox_inches='tight', dpi=1000, transparent=True)
        plt.savefig(f"{base_path}_pred.pdf", bbox_inches='tight', dpi=1000, transparent=True)

        return features_2d, true_labels, pred_labels

    def extract_features(self, eval_dest='eval', layer_name='feat', save_path=None):
        ...
        # Save true labels plot
        fig_true, ax_true = plt.subplots(figsize=(12, 8))  # 
        # 
        ax_true.spines['top'].set_visible(False)
        ax_true.spines['right'].set_visible(False)
        ax_true.spines['bottom'].set_visible(False)
    ...
    def extract_features_withframe(self, eval_dest='eval', layer_name='feat', save_path=None):
        ...
        # 
        if len(features_2d) != len(true_labels):
            self.print_fn(f"ERROR: TSNE output length {len(features_2d)} doesn't match labels length {len(true_labels)}")
        ...
        plt.subplot(1, 2, 1)
        scatter_true = plt.scatter(df_true['x'], df_true['y'], 
                                  c=df_true['label'], cmap=cmap,
                                  alpha=0.7, s=7, 
                                  vmin=0, vmax=self.num_classes-1)
        plt.title(f'TSNE of {layer_name} Features (True Labels)')
        ...
        plt.subplot(1, 2, 2)
        scatter_pred = plt.scatter(df_pred['x'], df_pred['y'], 
                                  c=df_pred['label'], cmap=cmap,
                                  alpha=0.7, s=7,
                                  vmin=0, vmax=self.num_classes-1)
        plt.title(f'TSNE of {layer_name} Features (Predicted Labels)')
        plt.legend(handles=legend_elements, 
                   loc='center left', 
                   bbox_to_anchor=(1, 0.5),
                   title='Classes')
    ...
    #
        if save_path is None:
            save_path = os.path.join(self.args.save_dir, self.args.save_name, f"tsne_{layer_name}_{eval_dest}_comparison.png")
            save_path_pdf = os.path.join(self.args.save_dir, self.args.save_name, f"tsne_{layer_name}_{eval_dest}_comparison.pdf")
        else:
            save_path = os.path.splitext(save_path)[0] + '_comparison.png'
            save_path_pdf = os.path.splitext(save_path)[0] + '_comparison.pdf'
        ...
        plt.legend(handles=legend_elements, 
                   ...
                   title='Classes')
        plt.tight_layout()
        plt.savefig(f"{base_path}_true.png", bbox_inches='tight', dpi=1000)
        plt.savefig(f"{base_path}_true.pdf", bbox_inches='tight', dpi=1000)
        ...
        plt.legend(handles=legend_elements, 
                   ...
                   title='Classes')
        plt.tight_layout()
        plt.savefig(f"{base_path}_pred.png", bbox_inches='tight', dpi=1000)
        plt.savefig(f"{base_path}_pred.pdf", bbox_inches='tight', dpi=1000)
