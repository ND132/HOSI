...
class AlgorithmBase:
    ...
    def __init__(self, args, net_builder, tb_log=None, logger=None, **kwargs):
        ...
        self.imb_algorithm = args.imb_algorithm
    ...
    def set_data_loader(self):
        ...
        loader_dict['train_ulb_all'] = get_data_loader(self.args,
                                                   copy.deepcopy(self.dataset_dict['train_ulb']),
                                                   self.args.batch_size * self.args.uratio,
                                                   data_sampler=self.args.train_sampler,
                                                   num_iters=self.num_train_iter,
                                                   num_epochs=self.epochs,
    ...
    def process_batch_sco(self, input_args=None, **kwargs):
        """
        process batch data, send data to cuda
        NOTE **kwargs should have the same arguments to train_step function as keys to work properly
        """
        if input_args is None:
            input_args = signature(self.train_step_sco).parameters
            input_args = list(input_args.keys())
    ...
    def train(self):
        ...
        #self.train_ulb_len = len(self.dataset_dict['train_ulb'])
        if self.imb_algorithm == 'scomatch':
            for epoch in range(self.start_epoch, self.epochs):
                ...
                for data_lb, data_ulb, data_ulb_all in zip(self.loader_dict['train_lb'],
                                            self.loader_dict['train_ulb'],self.loader_dict['train_ulb_all']):
                    # prevent the training iterations exceed args.num_train_iter
                    if self.it >= self.num_train_iter:
                        break
