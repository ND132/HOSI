...
def split_labeled_unlabeled_data(args, data, targets,train_data_img, train_targets_img, num_classes, num_ood_classes, lb_num_labels, ulb_num_labels=None,
    ...
    lb_idx, ulb_idx, ulb_ood_idx, lb_clean_idx, lb_noise_idx = sample_labeled_unlabeled_data(args, data, targets,
                                                                                train_data_img, train_targets_img,
                                                                                num_classes,num_ood_classes,
    ...
    if include_lb_to_ulb:
        ulb_idx = np.concatenate([lb_idx, ulb_idx, ulb_ood_idx], axis=0)

    return lb_idx, ulb_idx, ulb_ood_idx, lb_clean_idx, lb_noise_idx


def sample_labeled_unlabeled_data(args, data, target, train_data_img, train_targets_img, num_classes, num_ood_classes, lb_num_labels, ulb_num_labels=None,
                                  lb_imbalance_ratio=1.0, ulb_imbalance_ratio=1.0, noise_ratio=0.1,
    ...
    ulb_ood_dump_path = os.path.join(dump_dir,
                                 f'ulb_ood_labels_{args.num_labels}_{args.lb_imb_ratio}_{args.ulb_num_labels}_{args.ulb_imb_ratio}_{args.ulb_imb_type}_noise_{args.noise_ratio}_seed_{args.seed}_idx.npy')
    ...
    flag = os.path.exists(lb_dump_path) and os.path.exists(ulb_dump_path) and os.path.exists(
        lb_clean_dump_path) and os.path.exists(lb_noise_dump_path) and os.path.exists(ulb_ood_dump_path)
    if flag and load_exist:
        lb_idx = np.load(lb_dump_path)
        ulb_idx = np.load(ulb_dump_path)
        ulb_ood_idx = np.load(ulb_ood_dump_path)
    ...
    if -1 in target or -1 in train_targets_img:
        ...
        if ulb_imbalance_ratio == 1.0:
            ...
            if ulb_num_labels is None or ulb_num_labels == 'None':
                ulb_samples_per_ID_class = [int(len(data) / num_classes) - lb_samples_per_class[c] for c in range(num_classes)]
                ulb_samples_per_OOD_class = [int(len(train_data_img) / num_ood_classes)] * num_ood_classes
                ulb_samples_per_class = ulb_samples_per_ID_class + ulb_samples_per_OOD_class
                # [int(len(data) / num_classes) - int(lb_num_labels / num_classes)] * num_classes
            else:
                #assert ulb_num_labels % ( num_classes + num_ood_classes ) == 0, "ulb_num_labels must be dividable by num_classes in balanced setting"
                #ulb_samples_per_class = [int(ulb_num_labels / ( num_classes + num_ood_classes ))] * ( num_classes + num_ood_classes )
                #ulb_samples_per_class = [int(ulb_num_labels)] * ( num_classes + num_ood_classes )
                ulb_samples_per_ID_class = [int(ulb_num_labels)] * num_classes
                ulb_samples_per_OOD_class = [int(ulb_num_labels * 0.2)] * num_ood_classes
                ulb_samples_per_class = ulb_samples_per_ID_class + ulb_samples_per_OOD_class
        else:
            # imbalanced setting
            assert ulb_num_labels is not None, "ulb_num_labels must be set set in imbalanced setting"
            ulb_samples_per_class = make_imbalance_data(args, ulb_num_labels, (num_classes + num_ood_classes), ulb_imbalance_ratio, ulb_imb_type, num_steps)
            random.shuffle(ulb_samples_per_class)
        ...
        for c in range(num_ood_classes):
            idx = np.where(train_targets_img == c+num_classes)[0]
            np.random.shuffle(idx)

            if ulb_num_labels is None or ulb_num_labels == 'None':
                ulb_ood_idx.extend(idx)
            else:
                ulb_ood_idx.extend(idx[:ulb_samples_per_class[num_classes+c]])
    ...
    if isinstance(ulb_ood_idx, list):
        ulb_ood_idx = np.asarray(ulb_ood_idx)

    if isinstance(lb_clean_idx, list):
        lb_clean_idx = np.asarray(lb_clean_idx, dtype=int)
    ...
    np.save(ulb_ood_dump_path, ulb_ood_idx)
