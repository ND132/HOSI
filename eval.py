...
def get_config():
    ...
    # 
    parser.add_argument('--logit_adjustment', type=str, default=None,choices=[None, 'subtract', 'divide'],help='logit adjustment method (None/subtract/divide)')
    # 
    parser.add_argument('--tau', type=float, default=1.0, help='logit adjustment strength (default: 1.0)')
    ...
    parser.add_argument("-noc", "--num_ood_classes", type=int, default=10)
    ...
    args = parser.parse_args()
    over_write_args_from_file(args, args.c)
    print("Registered imbalanced algorithms:", list(name2imbalg.keys()))
...
def main_worker(gpu, ngpus_per_node, args):
    ...
    #         logger.error("Resume training requires --load_path to be specified")
    #         raise ValueError("--load_path must be provided when --resume is set")
    
    #     if not os.path.exists(args.load_path):
    #         logger.error(f"Model checkpoint file not found: {args.load_path}")
    #         raise FileNotFoundError(f"Checkpoint file does not exist: {args.load_path}")
    
    #     try:
    #         logger.info(f"Attempting to load model from: {args.load_path}")
    #         model.load_model(args.load_path)
    #         logger.info(f"Successfully loaded model checkpoint from {args.load_path}")
        
    #     except Exception as e:
    #         logger.error(f"Failed to load model checkpoint from {args.load_path}")
    #         logger.error(f"Error type: {type(e).__name__}")
    #         logger.error(f"Error details: {str(e)}")
        
    #         # 
    #         if isinstance(e, RuntimeError) and "Missing key(s)" in str(e):
    #             logger.error("Model architecture mismatch detected")
    #             # 
            
    #         # 
    #         elif isinstance(e, RuntimeError) and "module." in str(e):
    #             logger.error("Possible multi-GPU vs single-GPU model mismatch")
    #             logger.error("Try loading with DataParallel wrapper or removing 'module.' prefix")
            
    #         # 
    #         elif isinstance(e, (pickle.UnpicklingError, zipfile.BadZipFile)):
    #             logger.error("Possible PyTorch version incompatibility")
    #             logger.error("Try saving/loading with the same PyTorch version")
            
    #         args.resume = False
    #         raise  # 


    # START Tsne of FixMatch
    logger.info("Model evaluate")
    model.evaluate()

    

    logging.warning(f"GPU {args.rank} tsne is FINISHED")
